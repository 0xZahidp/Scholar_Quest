...
import { getProfessors, upsertProfessor, deleteProfessor, PROF_STATUSES } from "@/lib/professors.functions";
...
type Form = {
  id?: string; name: string; university: string; field: string; email: string;
  status: typeof PROF_STATUSES[number]; last_contact_on: string; notes: string;
};
const empty: Form = { name: "", university: "", field: "", email: "", status: "researching", last_contact_on: "", notes: "" };
...
function ProfessorsPage() {
  ...
  const delMut = useMutation({
    ...
    onSuccess: () => { toast.success("Removed"); qc.invalidateQueries({ queryKey: ["professors"] }); },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-3">
        <div>
          <div className="text-xs uppercase tracking-widest text-muted-foreground">Outreach Network</div>
          <h1 className="font-display text-3xl font-bold">Professors</h1>
          <p className="mt-1 text-sm text-muted-foreground">Cold emails turn into supervisors. Track every contact.</p>
        </div>
        <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setForm(empty); }}>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader><DialogTitle>{form.id ? "Edit" : "New"} professor</DialogTitle></DialogHeader>
            <div className="grid gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <Field label="Research field"><Input value={form.field} onChange={(e) => setForm({ ...form, field: e.target.value })} /></Field>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Email"><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></Field>
                <Field label="Last contact">
                  <Input type="date" value={form.last_contact_on} onChange={(e) => setForm({ ...form, last_contact_on: e.target.value })} />
                </Field>
              </div>
              <Field label="Status">
                <Select value={form.status} onValueChange={(v) => setForm({ ...form, status: v as Form["status"] })}>
                  ...
                  <SelectContent>
                    {PROF_STATUSES.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                  </SelectContent>
                </Select>
              </Field>
              <Field label="Notes"><Textarea rows={3} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></Field>
              <Button onClick={() => saveMut.mutate()} disabled={!form.name || saveMut.isPending}
                className="bg-gradient-to-r from-primary to-accent">Save contact</Button>
            </div>
          </DialogContent>
        </Dialog>
      </div>
      ...
      {q.isLoading ? null : !q.data || q.data.length === 0 ? (
        <GlassCard>
          <div className="flex flex-col items-center gap-3 p-8 text-center">
            ...
            <p className="max-w-sm text-sm text-muted-foreground">One reply from a future supervisor can change everything.</p>
          </div>
        </GlassCard>
      ) : (
            ...
            <GlassCard key={p.id} delay={i * 0.04}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {p.email && (
                    <a href={`mailto:${p.email}`} className="mt-2 inline-flex items-center gap-1 text-xs text-primary-glow hover:underline">
              ...
              <div className="mt-3 flex items-center justify-between border-t border-border/40 pt-3 text-xs text-muted-foreground">
                <span>{p.last_contact_on ? `Last contact: ${p.last_contact_on}` : "Not yet contacted"}</span>
                <div className="flex gap-1">
                  <Button size="icon" variant="ghost" onClick={() => {
                    setForm({
                      id: p.id, name: p.name, university: p.university ?? "", field: p.field ?? "",
                      email: p.email ?? "", status: p.status as Form["status"],
                      last_contact_on: p.last_contact_on ?? "", notes: p.notes ?? "",
                    });
                    setOpen(true);
